...
public class Terrain {
	...
	public float getHeightOffTerrain(float worldX, float worldZ) {
	...
	private RawModel generateTerrain(Loader loader, String heightMap){
		...
		for(int i=0;i<vertexCount;i++){
			for(int j=0;j<vertexCount;j++){
				...
				Vector3f normal = calcMormal(j, i, image);
	...
	private Vector3f calcMormal(int x, int z, BufferedImage heightMap) {
	...
	private float getHeight(int x, int z, BufferedImage heightMap) {
		if(x<0||x>=heightMap.getHeight()||z<0||z>=heightMap.getHeight()) return 0;
